    ...
    *,
    ...
    if normalized_type == "quantity_proportional":
        ...
        if first_quantity <= 0 or additional_quantity <= 0:
            return result
        bundle_count = max(1, math.ceil(max(quantity, 1) / additional_quantity))
        result.update(
            {
                "shipping_type": "quantity_proportional",
                "shipping_fee": first_fee + Decimal(bundle_count - 1) * additional_fee,
    ...
    if normalized_type == "quantity_tiered":
        ...
        selected_fee = None
        for rule in rules:
            if max(quantity, 1) >= rule["min_quantity"]:
                selected_fee = rule["fee"]
        result.update(
            {
