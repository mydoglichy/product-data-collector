...
디버깅용 제한 raw sample을 저장합니다. 전체 API 응답을 장기 보관하기 위한 주 저장소가 아니라, 문제 분석에 필요한 일부 원본 payload 보관용입니다. 플랫폼별 보관 수는 최대 3개이며 새 sample 저장 후 초과분은 최신 `collected_at` 기준으로 삭제합니다.
